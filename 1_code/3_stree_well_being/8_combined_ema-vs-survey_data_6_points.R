...
dir.create(
  "3_data_analysis/3_stress_and_well_being/8_combined_ema-vs-survey_data_6_points",
  recursive = TRUE
)
setwd(
  "3_data_analysis/3_stress_and_well_being/8_combined_ema-vs-survey_data_6_points"
...
ema_expression_data <-
  ema_expression_data[-6, ] %>%
  t() %>%
  as.data.frame() %>%
  dplyr::mutate(ema_mean = rowMeans(across(everything()), na.rm = TRUE)) %>%
  dplyr::select(ema_mean) %>%
  t() %>%
  as.data.frame()

ema_variable_info <-
  ema_variable_info %>%
  dplyr::filter(variable_id == "ema_complete")

ema_variable_info$variable_id <-
  ema_variable_info$real_name <-
  "ema_mean"
...
#             pull(sample_id2)
#           ema_temp_data[temp_sample_id2, , drop = FALSE] %>%
#             as.data.frame() %>%
#             colMeans()
#         }) %>%
#         do.call(rbind, .) %>%
...
#       
#       temp_data1 <-
#         survey_temp_data[unique(x$sample_id1), , drop = FALSE]
#       
#       cbind(temp_data1, temp_data2) %>%
#         dplyr::mutate(subject_id = x$subject_id[1]) %>%
#         dplyr::mutate(time_point = unique(x$time_point1))
#       
#     }
#     
#   })
...
cor_data_individual <-
  ...
  purrr::map(function(x) {
    ...
    temp_cor <-
      Hmisc::rcorr(as.matrix(x %>% dplyr::select(-c(subject_id, time_point))), type = "spearman")
...
cor_data_individual$name1[cor_data_individual$name1 %in% ema_variable_info$real_name] <-
  paste0("ema_", cor_data_individual$name1[cor_data_individual$name1 %in% ema_variable_info$real_name])

cor_data_individual$name2[cor_data_individual$name2 %in% ema_variable_info$real_name] <-
  paste0("ema_", cor_data_individual$name2[cor_data_individual$name2 %in% ema_variable_info$real_name])
...
id =
  final_result %>% lapply(function(x) {
    id = x$subject_id[1]
    if (is.null(id)) {
      id = "NA"
    }
...
temp$time_point <- as.character(temp$time_point)

cor.test(temp$Stress_PSS, temp$ema_mean, method = "spearman")

cor.test(temp$Burnout_total_OLBI, temp$ema_mean, method = "spearman")

cor.test(temp$Resilience_CDRISC, temp$ema_mean, method = "spearman")
...
temp_data <-
temp %>% 
  dplyr::select(-c(subject_id, time_point, ema_mean)) %>% 
  colnames() %>% 
  purrr::map(function(x){
    test <-
      cor.test(temp$ema_mean, temp[[x]], method = "spearman")
    data.frame(from = x, 
               to = "ema_mean",
               cor = test$estimate,
               p = test$p.value)
...
temp_data %>% 
  dplyr::arrange(cor) %>% 
  dplyr::mutate(from = factor(from, levels = from)) %>%
  dplyr::mutate(class = ifelse(cor > 0, "positive", "negative"))

plot <-
temp_data %>%
  ggplot(aes(y = from, x = cor)) +
  geom_segment(aes(xend = 0, yend = from,
                   color = class), size = 0.5) +
  geom_point(aes(size = -log(p, 10),
                 color = class)) +
  labs(y = "", x = "Spearman correlation") +
  ...
  geom_text(aes(x = 0,
                y = from,
                label = from),
            hjust = ifelse(temp_data$cor > 0, 1.1, 0),
            nudge_x = 0.01)

plot

ggsave(plot, filename = "ema_vs_survey_data.pdf", width = 7, height = 8)
